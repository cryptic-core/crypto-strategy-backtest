...
def run_liquidation_backtest(df):
    ...
    for index, row in df.iterrows():
        ...
        if not in_buy_cycle and row['bLMax']:
...
if __name__ == '__main__':
    ...
    # Read and process both dataframes
    df = pd.read_csv(os.getcwd()+'/data/candlestick_data/BTC-USDT.csv')
    df2 = pd.read_csv(os.getcwd()+'/data/liquidation_data/liquidation.csv')
    
    # Convert date columns to datetime
    df['date'] = pd.to_datetime(df['date'])
    df2['date'] = pd.to_datetime(df2['date'])
    
    # Filter both dataframes from start_date
    df = df[df['date'] >= start_date]
    df2 = df2[df2['date'] >= start_date]
    
    # Merge the filtered dataframes
    df = pd.merge(df, df2, on='date', how='left')
    df = run_liquidation_backtest(df)
